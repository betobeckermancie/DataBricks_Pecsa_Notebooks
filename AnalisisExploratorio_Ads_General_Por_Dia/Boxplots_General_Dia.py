...
# MAGIC
# MAGIC # Cargar el csv para analizar
# MAGIC df = pd.read_csv("/dbfs/mnt/processed/Ads_General_Por_Dia/anuncios_insights_general_por_dia_limpiado.csv")
# MAGIC
# MAGIC # Agrupar los anuncios por nombre y sumar las interacciones
# MAGIC df_grouped = df.groupby('nombre_anuncio', as_index=False).sum(numeric_only=True)
# MAGIC
# MAGIC # Ordenar los anuncios de mayor a menor por la columna 'vistas_mensaje_bienvenida '
# MAGIC df_sorted = df_grouped.sort_values(by='vistas_mensaje_bienvenida', ascending=False)
# MAGIC
# MAGIC # Configurar las propiedades de los fliers (valores atípicos)
# MAGIC flierprops = dict(marker='o', markerfacecolor='red', markersize=5, linestyle='none')
# MAGIC
# MAGIC # Crear el gráfico a detalle
# MAGIC plt.figure(figsize=(5,6))  # Ajustar el tamaño del gráfico
# MAGIC
# MAGIC # Creando boxplot detallado con los datos agrupados
# MAGIC box = plt.boxplot(df_sorted['vistas_mensaje_bienvenida'], patch_artist=True, notch=True, vert=True, showmeans=True, flierprops=flierprops)
# MAGIC
# MAGIC # Personalizar colores de la caja
# MAGIC for patch in box['boxes']:
# MAGIC     patch.set_facecolor('lightblue')
# MAGIC # Personalizar colores de los bigotes y medianas
# MAGIC for whisker in box['whiskers']:
# MAGIC     whisker.set(color='black', linewidth=1.5)
# MAGIC # Gorras
# MAGIC for cap in box['caps']:
# MAGIC     cap.set(color='black', linewidth=1.5)
# MAGIC # Mediana
# MAGIC for mean in box['means']:
# MAGIC     mean.set(marker='o', color='green', markersize=5)
# MAGIC
# MAGIC # Etiquetas y título
# MAGIC plt.title('Distribución de vistas al msj de bienvenida', fontsize=16)
# MAGIC plt.ylabel('Num Vistas', fontsize=16)
# MAGIC plt.grid(True, linestyle='--', alpha=1)
# MAGIC
# MAGIC # Ajustar el rango del eje 'Y' y los ticks
# MAGIC #plt.ylim(0, df_grouped['vistas_mensaje_bienvenida'].max()+500)  # Ajustar el límite superior del eje Y
# MAGIC #plt.yticks(range(0, int(df_grouped['vistas_mensaje_bienvenida '].max())+1000, 1000))  # Configurar los ticks de 1000 en 1000
# MAGIC
# MAGIC # Mostrar gráfico
# MAGIC plt.show()
# MAGIC
# MAGIC # Identificar límites del boxplot
# MAGIC q1 = df_sorted['vistas_mensaje_bienvenida'].quantile(0.25)
# MAGIC q3 = df_sorted['vistas_mensaje_bienvenida'].quantile(0.75)
# MAGIC iqr = q3 - q1
# MAGIC lower_bound = q1 - 1.5 * iqr
# MAGIC upper_bound = q3 + 1.5 * iqr
# MAGIC
# MAGIC # Filtrar anuncios con más vistas_mensaje_bienvenida  por encima del límite superior
# MAGIC anuncios_outliers = df_sorted[df_sorted['vistas_mensaje_bienvenida'] > upper_bound]
# MAGIC
# MAGIC # Mostrar los nombres de los anuncios que son outliers
# MAGIC print("Anuncios con su valor fuera del rango (outliers):")
# MAGIC print(anuncios_outliers[['nombre_anuncio', 'vistas_mensaje_bienvenida']])
...
# Cargar el csv para analizar Ads_General_Por_Dia/anuncios_insights_general_por_dia_limpiado.csv
df = pd.read_csv("/dbfs/mnt/processed/Ads_General_Por_Dia/anuncios_insights_general_por_dia_limpiado.csv")
